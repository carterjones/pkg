...
docker build -f Dockerfile -t carterjones/git:latest --build-arg VERSION="${VERSION}" .
...
docker run -d -v $(pwd)/scratch:/tmp/scratch carterjones/git:latest /bin/bash -c "cd /build/git/git && make install && touch /tmp/done-with-build && sleep infinity"
...
mv ./scratch/git-files.tar.gz "./scratch/git-${VERSION}.tar.gz"

# Upload to S3.
aws s3api put-object \
    --acl public-read \
    --bucket res.carterjones.info \
    --key "pkg/centos7/git/git-${VERSION}.tar.gz" \
    --body "./scratch/git-${VERSION}.tar.gz"
