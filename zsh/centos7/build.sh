...
docker build -f Dockerfile -t carterjones/zsh:latest --build-arg VERSION="${VERSION}" .
...
docker run -d -v $(pwd)/scratch:/tmp/scratch carterjones/zsh:latest /bin/bash -c "cd /build/zsh/zsh && make install.bin && make install.modules && make install.fns && touch /tmp/done-with-build && sleep infinity"
...
mv ./scratch/zsh-files.tar.gz "./scratch/zsh-${VERSION}.tar.gz"

# Upload to S3.
aws s3api put-object \
    --acl public-read \
    --bucket res.carterjones.info \
    --key "pkg/centos7/zsh/zsh-${VERSION}.tar.gz" \
    --body "./scratch/zsh-${VERSION}.tar.gz"
